...
# Assignment: PA4
# This assignment implements all the load
# routines.
#-------------------------------------------------------------------------
...
optbl:
	...
	.long srspdi		#03 Robot Speed (Rd,Iv)
...
srsns:
	pushl $rsnscmd			# 
	pushl $sregs+(8*4)		#
	call  sndRecv_0			# get sensor data
	addl  $8,%esp			# 
	addl  $1,%edi			# point IP to next op-code
	ret
...
#-----------------------------------------------------------------------
srspdr:
	movb  smem+1(,%edi,1),%al	# get Rd and Rs indexes
	andl  0xff,%eax			# clear high-order bits
...
#-----------------------------------------------------------------------
srspdi:
	movl  smem+2(,%edi,1),%eax	# get Iv data
	movl  smem+6(,%edi,1),%ebx	# get next Iv data
	pushl %ebx			#
	pushl %eax			#
	pushl $rspdcmd			#
	pushl $rspdbuf			#
